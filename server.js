...
function ensureInventoryShape(inv) {
  inv.items ||= {};
  inv.family ||= { weapons: {}, leichteWesten: 0, schwereWesten: 0, munition: 0, history: [] };
  inv.family.weapons ||= {};
  inv.family.history ||= [];
  return inv;
}
app.post('/api/inventory/family', allowTokenOrLogin, requireAction('cashboxWrite'), (req, res) => {
  ...
  for (const k of ['leichteWesten','schwereWesten','munition']) if (k in patch) st.inventory.family[k] = Math.max(0, Number(patch[k] || 0));
